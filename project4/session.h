...
private:
    ...
    */
    ...
    string get_program_name(Env env){
        ...
        if(pos >= 0){
            program = env["REQUEST_URI"].substr(1,pos-1).c_str();
        }
        else{
            program = env["REQUEST_URI"].substr(1).c_str();
        }
    ...
    void send_handler(const ErrCode& ec, size_t length){
        if(!ec){
            // local_endpoint(): getsocketname()
            // remote_endpoint(): getpeername()->acceptor socket
